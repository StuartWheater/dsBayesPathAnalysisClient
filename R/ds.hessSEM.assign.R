...
#' @import DSI
#' @export
ds.hessSEM.assign <- function(resource.name, inFile, blockList.name, varType.name, SEMGraph.name, autoAddIntercept, gammaInit, nIter, burnin, nChains, seed, method, newobj=NULL, datasources=NULL){
    ...
    calltext <- call("hessSEM.assignDS", as.symbol(resource.name.transfer), inFile.transfer, as.symbol(blockList.name.transfer), as.symbol(varType.name.transfer), as.symbol(SEMGraph.name.transfer), autoAddIntercept.transfer, gammaInit.transfer, nIter.transfer, burnin.transfer, nChains.transfer, seed.transfer, method.transfer)
